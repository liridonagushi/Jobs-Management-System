...
// Total companies
$total = count_companies($_SESSION[$filename]['search_query'], $my_companies=false);
...
<div class="container">
  <div class="row">
    <div class="col-md-12 text-center">
      <h4 class="padding10"><b><i class="fa fa-file-o"></i> All Companies</b></h4>
...
<section id="module" class="bg-gray">
  ...
  <div class="container-custom-grid">
   <div class="row">
    ...
    <?php $comp=$dbj->query('SELECT companies.id_company, employers.name, employers.surname, employers.email, companies.company_name, companies.company_sn, category_expertises.'.$_SESSION['jbms_front']['lang_code'].'_expertise_category AS exp_field, adresses.adress, adresses.city, adresses.postal_code, countries.country_code, countries.country_name, companies.active FROM companies LEFT JOIN adresses ON companies.id_adress=adresses.id_adress LEFT JOIN category_expertises ON companies.id_cat_expertise=category_expertises.id_cat_expertise LEFT JOIN countries ON adresses.id_country=countries.id_country LEFT JOIN users AS employers ON (companies.id_employer=employers.id_user AND employers.admin_level="2") WHERE 1=1 '.$_SESSION[$filename]['search_query'].' '.$_SESSION[$filename]['orderby'].' LIMIT '.$start.', '.$_SESSION['epp'].'');
    ?>
    ...
    <div class="wizard-card row border padding30">
      ...
      <div class="form-group col-lg-2">
        <h4 class="iconbox-header">Employer</h4>
        <fieldset>
          <?php echo $row_comp['name'];?>
          <?php echo $row_comp['surname'];?>
        </fieldset>
        <fieldset>
        <?php echo $row_comp['email'];?>
        </fieldset>
      </div>
      ...
      <div class="form-group col-lg-2">
        <h4 class="iconbox-header">Adress</h4>
        <fieldset>
